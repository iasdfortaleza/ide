...
import { 
  CalendarCheck, BookHeart, Users, Send, Filter, 
...
// Definindo as tipagens que chegam do servidor
...
export default function LancamentosClient({
  ...
  // Função central para executar as actions e disparar o Toast
  const executarAcao = async (formData: FormData, actionFn: any) => {
  ...
  const confirmarExclusao = (formData: FormData, actionFn: any, itemDescricao: string) => {
    setModal({
      isOpen: true,
      ...
      onConfirm: async () => {
        fecharModal();
        await executarAcao(formData, actionFn);
      }
    });
  };
            ...
            <details key={dupla.id} className="group border border-border bg-card/80 backdrop-blur-md rounded-xl overflow-hidden shadow-lg [&_summary::-webkit-details-marker]:hidden">
              ...
              <summary className="bg-muted/10 px-6 py-4 border-b border-border flex justify-between items-center cursor-pointer list-none hover:bg-muted/30 transition-colors">
                <div className="flex items-center gap-4">
                  <FotoModal url_foto_dupla={dupla.url_foto_dupla} nome_dupla={dupla.nome_dupla} />
              ...
              <div className="flex flex-col divide-y divide-border">
                                ...
                                return (
                                  <form action={(fd) => executarAcao(fd, editarLancamentoEstudo)} key={`${registro.id}-${registro.licao_id}-${registro.data_registro}`} className="flex items-center gap-2 p-2 bg-card rounded-lg border border-border shadow-sm">
                                    ...
                                    <div className="ml-auto flex gap-1">
                                      ...
                                      <button 
                                        type="button" // Mudado para type="button" para interceptar
                                        onClick={(e) => {
                ...
                <div className="p-4 md:p-6 bg-card/60">
                  ...
                  <div className="grid lg:grid-cols-2 gap-8">
                    ...
                    <form action={(fd) => executarAcao(fd, lancarVisita)} className="p-5 bg-card border border-border rounded-xl space-y-5 shadow-sm h-fit">
                    ...
                    <div className="bg-background/40 border border-border p-4 rounded-xl shadow-inner h-fit">
                      <details className="group/histVisita" suppressHydrationWarning>
                        ...
                        <div className="mt-4 space-y-2 max-h-64 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-primary/20">
                          {historicoVisitas.length === 0 ? (
                            ...
                            historicoVisitas.map((visita: any) => {
                              return (
                                <form action={(fd) => executarAcao(fd, editarVisita)} key={`${visita.id}-${visita.nome_visitado}-${visita.data_visita}`} className="flex flex-wrap items-center gap-2 p-2 bg-card rounded-lg border border-border shadow-sm">
                                  ...
                                  <div className="flex gap-1 ml-auto">
                                    ...
                                    <button 
                                      type="button" // Mudado para interceptar clique
                                      onClick={(e) => {
                                        const form = e.currentTarget.closest('form');
                                        if (form) {
                                          const fd = new FormData(form);
                                          confirmarExclusao(fd, excluirVisita, "este registro de visita");
                                        }
                                      }}
                                      className="p-1.5 hover:bg-destructive/20 rounded text-destructive transition-colors"
                                      title="Excluir"
