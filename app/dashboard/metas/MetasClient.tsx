...
// Definição das propriedades recebidas do Server Component
...
export default function MetasClient({ pelotoes, metas, anoAtual, maxAno }: MetasClientProps) {
  ...
  // Função para lidar com formulários (Criar/Editar)
  const executarAcaoForm = async (formData: FormData, actionFn: any, isCreate = false) => {
    const result = await actionFn(formData);
    if (result.success) {
      addToast(result.message, "success");
      // Se foi criação, limpa apenas os campos de alvo, mas mantém o pelotão e o ano
      if (isCreate && formRef.current) {
        const form = formRef.current;
        (form.elements.namedItem('alvo_anual') as HTMLInputElement).value = '';
        (form.elements.namedItem('alvo_alcancado') as HTMLInputElement).value = '0';
      }
    } else {
  ...
  const confirmarExclusao = (id: string, actionFn: any, itemDescricao: string) => {
    setModal({
      ...
      message: `Tem certeza que deseja excluir ${itemDescricao}? Os dados históricos desta meta serão perdidos.`,
      isDestructive: true,
      onConfirm: async () => {
        fecharModal();
          ...
          <Card className="border-border bg-card/80 backdrop-blur-md sticky top-24 shadow-xl">
            ...
            <CardContent className="pt-5">
              {pelotoes && pelotoes.length > 0 ? (
                <form ref={formRef} action={(fd) => executarAcaoForm(fd, salvarMeta, true)} className="space-y-4">
                      ...
                      <div className="mt-auto space-y-2 pt-4">
                        <details className="group/edit [&_summary::-webkit-details-marker]:hidden">
                          ...
                          <div className="pt-4 space-y-3 pb-2">
                            <form action={(fd) => executarAcaoForm(fd, editarMeta)} className="space-y-3 bg-muted/5 p-4 rounded-lg border border-border/50 shadow-inner">
                        ...
                        <button 
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            confirmarExclusao(meta.id, excluirMeta, `a meta do pelotão ${nomePelotao} do ano de ${meta.ano}`);
                          }}
                          className="flex items-center justify-center w-full h-9 gap-2 text-sm font-medium rounded-md text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors"
                        >
