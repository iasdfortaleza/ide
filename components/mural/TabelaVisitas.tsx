        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-muted/45 text-muted-foreground uppercase text-[9px] md:text-[10px] tracking-[0.18em] font-bold border-b border-border/50">
              <th className="px-3 py-2 text-left w-1/4">Data</th>
              <th className="px-3 py-2 text-left w-1/3">Dupla Missionária</th>
              <th className="px-3 py-2 text-left">Família / Visitado</th>
            </tr>
          </thead>
              ...
              visitasLancadas.map((visita, index) => {
                ...
                return (
                  <tr key={visita.id} className={`${isEven ? 'bg-background/35' : 'bg-muted/10'} hover:bg-muted/30 transition-colors border-b border-border/35`}>
                    ...
                    <td className="px-3 py-1.5 font-bold text-foreground/80 truncate max-w-[200px]">
                      {dupla?.nome_dupla || "Desconhecida"}
                    </td>
                    <td className="px-3 py-1.5 font-semibold text-primary truncate max-w-[200px]">
