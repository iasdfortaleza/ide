...
export function MuralHeader({ nome, igreja, startDate, endDate }: MuralHeaderProps) {
  ...
  const startFormatado = startDate ? new Date(startDate).toLocaleDateString('pt-BR', { timeZone: 'UTC' }) : '';
  const endFormatado = endDate ? new Date(endDate).toLocaleDateString('pt-BR', { timeZone: 'UTC' }) : '';
  ...
  return (
    <header className="w-full bg-card/90 backdrop-blur-xl border-b border-border/50 px-4 md:px-8 py-3 flex items-center justify-between sticky top-0 z-50 shadow-sm">
      ...
      {/* ÁREA DIREITA: PERÍODO + CALENDÁRIO */}
      <div className="flex items-center gap-3">
        
        {/* EXIBIÇÃO DO PERÍODO SELECIONADO EMPILHADO */}
        <div className="hidden sm:flex flex-col items-end text-[9px] text-muted-foreground font-bold uppercase tracking-widest leading-tight">
          <span>De: {startFormatado}</span>
          <span>Até: {endFormatado}</span>
        </div>
