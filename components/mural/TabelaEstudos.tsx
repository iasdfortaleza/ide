...
export function TabelaEstudos({
  ...
  const getStatusAluno = (estudanteId: string, livroId: string) => {
    ...
    // Cores limpas: Os blocos não preenchidos assumem bg-muted (translucidez elegante sobre o azul)
    let color = "bg-muted"; 
    // Cores padrão do Tailwind (Yellow, Orange, Blue, Green) ficam maravilhosas sobre o azul!
    ...
    else if (porcentagem > 66 && porcentagem < 100) activeColor = "bg-blue-400 shadow-[0_0_5px_rgba(96,165,250,0.5)]";
  ...
  return (
    <>
      ...
      {fotoExpandida && (
        ...
        >
          {/* Removido o HEX hardcoded rgba(212,175,55) da sombra. Passamos a usar a var(--color-primary) ou class tailwind nativa */}
      ...
      <details className="group bg-card/60 backdrop-blur-md border border-border/50 rounded-xl overflow-hidden shadow-xl shadow-black/10 [&_summary::-webkit-details-marker]:hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse min-w-[700px]">
            <thead>
              <tr className="bg-muted/50 text-muted-foreground uppercase text-[9px] md:text-[10px] tracking-widest font-bold">
                <th className="p-3 text-left w-1/4">Dupla Missionária</th>
                <th className="p-3 text-left w-1/5">Aluno / Família</th>
                <th className="p-3 text-center">Progresso Geral</th>
                <th className="p-3 text-center w-24 border-r border-border/50">Lição Anterior</th>
                <th className="p-3 text-center w-24 bg-primary/5">Lição Atual</th>
              </tr>
            </thead>
            <tbody className="text-xs">
              {duplasFiltradas.length === 0 ? (
                ...
                duplasFiltradas.map((dupla, dIndex) => {
                  ...
                  // Ajuste sutil nas linhas para contrastarem elegantemente sobre o fundo azul/card escuro
                  ...
                  return dupla.estudantes.map((aluno: any, index: number) => {
                    ...
                    return (
                      <tr key={aluno.id} className={`${rowBgClass} ${borderClass} hover:bg-primary/5 transition-colors`}>
                        ...
                        {index === 0 ? (
                          <td rowSpan={dupla.estudantes.length} className="px-3 py-2 align-middle border-l-4 border-l-primary">
                            <div className="flex items-center gap-3">
                              <div 
                              ...
                              <div className="flex flex-col">
                                <span className="font-bold text-[11px] md:text-xs text-foreground leading-tight">
                                  {dupla.nome_dupla}
                                </span>
                                {integrantesStr && (
                                  <span className="text-[9px] text-muted-foreground italic mt-0.5 leading-tight">
                                    {integrantesStr}
                                  </span>
                                )}
                              </div>
                        ...
                        <td className="px-3 py-2 font-semibold text-[11px] md:text-xs text-foreground/90 truncate max-w-[150px]">{aluno.nome_pessoa}</td>
                        <td className="px-3 py-2 align-middle">
                          <div className="flex flex-wrap gap-[3px] items-center justify-center">
                            {blocos.map((preenchido, i) => (
                              <div 
                                key={i} 
                                className={`w-2.5 h-2.5 rounded-[1px] transition-all duration-500 ${preenchido ? activeColor : color}`}
                              />
                            ))}
                          </div>
                        </td>
                        ...
                        <td className="px-3 py-2 text-center border-r border-border/50 align-middle">
                           <div className="flex flex-col items-center justify-center leading-none">
                            <span className="font-bold text-xs md:text-sm text-foreground/80">{numeroLiAnterior}</span>
                            {dataLiAnterior && (
                              <span className="text-[9px] text-muted-foreground opacity-80 mt-1">
                                {dataLiAnterior}
                              </span>
                            )}
                          </div>
                        ...
                        <td className="px-3 py-2 text-center bg-primary/5 align-middle">
                           <div className="flex flex-col items-center justify-center leading-none">
                            <div className="flex items-baseline justify-center gap-0.5">
                              <span className="font-black text-sm md:text-base text-foreground">{maiorLicaoNumero}</span>
                              <span className="text-[9px] text-muted-foreground font-bold">/{totalLicoes}</span>
                            </div>
                            {dataLiAtual && (
                              <span className="mt-1 text-[9px] text-primary-foreground font-bold bg-primary/80 px-1.5 py-0.5 rounded border border-primary/20">
                                {dataLiAtual}
                              </span>
                            )}
                          </div>
          ...
          {/* LEGENDA */}
          <div className="bg-muted/30 p-2 border-t border-border/50 flex flex-wrap items-center justify-center gap-5 text-[9px] uppercase font-black tracking-widest text-muted-foreground opacity-80">
            <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-[1px] bg-yellow-500" /> Início</div>
            <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-[1px] bg-orange-500" /> Meio</div>
            <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-[1px] bg-blue-400" /> Quase lá</div>
            <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-[1px] bg-green-500" /> Pronto</div>
          </div>
        </div>
      </details>
    </>
